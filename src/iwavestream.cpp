...
bool iwavestream::read_frames(std::vector<float> *waves, uint32_t amount){
	...
	// Caution! no error protection for memory overflows.

	uint32_t buffz = amount*this->frameSize;
	char *buff = new char[buffz];
	
	uint32_t bsize = waves->size();

	amount *= this->channels;

	waves->resize(bsize+amount);
	...
	if(this->wavFile.bad() || this->wavFile.fail()){
		this->add_log("error reading file");
		return 0;
	}
